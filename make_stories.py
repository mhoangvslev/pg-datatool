...
if __name__ == '__main__':
    ...
    for idx, s in enumerate(articles):
        article_file = os.path.join(article_dir, "article_"+str(idx))
        summary_file = os.path.join(summary_dir, "summary_"+str(idx))
        
        article = open(article_file, 'r').read()
        summary = open(summary_file, 'r').read()

        with open(os.path.join(story_dir, "stories_"+str(idx)), "w") as file:
            story = article + '\n\n' + '@highlight'+'\n' + summary
            file.write(story)
